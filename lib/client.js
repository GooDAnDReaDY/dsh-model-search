...
/**
    ...
    function getLocalizedTitle() {
      if (typeof translate === 'function') {
        const val = translate('dms.title')
        if (val && val !== 'dms.title') return val
      }
      ...
      if (lang.startsWith('zh')) return I18N.zh.title
      ...
      const updateFilter = () => {
        ...
        for (const grp of sectionGroups) {
          ...
          for (const opt of options) {
            ...
            const match = matchesQuery(modelText, terms, groupTitle)
            if (match) {
              opt.style.display = ''
              opt.removeAttribute('aria-hidden')
              groupVisible++
    ...
    function scan() {
      ...
      try {
        for (const menu of Array.from(activeMenus)) {
          if (menu.isConnected === false || (typeof document.contains === 'function' && !document.contains(menu))) {
            detachSearch(menu)
          }
        }
    ...
    function isMenuNode(node) {
      ...
      return Boolean(
        ...
        node.closest?.('[role="menu"], [class*="_menu"]')
      )
    }
    ...
    function ModelSearchSettingsCard(props) {
      ...
      const currentVer = status?.currentVersion || '0.1.12'
